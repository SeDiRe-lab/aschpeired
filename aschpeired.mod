...
## Map bindings 
#timestep (seconds) 
TS              = 1;                        # time step duration in hours
...
###############################################################################
ErConst        = 0.001;     # Scour constant in erosion rate equation
DepConst       = 0.01;      # Deposition constant in deposition rate equation
...
RainDur      = 72;         # Rain duration in hours 72 hrx
    
                               
KsSoil        = 15.65;         # Hydraulic conductivity soil (mm/hour)
##############################################################################
######################################
...
#moisturecontent at beginning of the simulation volumetric (cm3/cm3)
 ...
 Moisturecont2=0.25;			# Values derived from 10 years simulation
 Moisturecont3=0.35;			# Values derived from 10 years simulation
 Moisturecont4=0.65;			# Values derived from 10 years simulation
...
# The rheology changes when it arrives in the plane due to consolidation effects 
    ...
    PhiDf=if(Dem gt 3580,40,35); #Bingham
    CohesDf=if(Dem gt 3580,1,5);#Bingham
    ViscosDf=if(Dem gt 3580,1,1);#Bingham
...
Precip=timeinput(Rain);
#Precip=if(Storm,Rain,0);
#Excess rain m/sec    
  ExceR=max(0.000000001,(Precip-KsSoil));
 #ExceR=(Precip-KsSoil);
...
#reports 
     ...
     report(rep_b) vel=Vel; #velocity report 
     ...
     report (rep_b) Df=if(CSF gt 0.2,H,0);
...
DepoDf1 =if(Dem gt 1700 and Dem le 1850,Dfdepo,0);
DepoDf2 =if(Dem gt 1850 and Dem le 2950,Dfdepo,0);
DepoDf3 =if(Dem gt 2950 and Dem le 3950,Dfdepo,0);
DepoDf4 =if(Dem gt 3950 and Dem le 4950,Dfdepo,0);
DepoDf5 =if(Dem gt 4950 and Dem le 5950,Dfdepo,0);
DepoDf6 =if(Dem gt 5950 and Dem le 6950,Dfdepo,0);
DepoDf7 =if(Dem gt 1850 and Dem le 6950,Dfdepo,0);
...
DepoVol7 =maptotal (DepoDf7*cellarea());
...
report FanVol7.tss=DepoVol6;
#report FanVol.tss=timeoutput(LsMonitor,Dfdepo);
...
report ExceR1.tss=timeoutput(LsMonitor,ExceR);
report ExceR2.tss=timeoutput(LsSample1,ExceR);  
report ExceR3.tss=timeoutput(LsSample2,ExceR);
